...
/obj/structure/mortar
	...
	unacidable = 1
	density = 1
	layer = ABOVE_HUMAN_LAYER //So you can't hide it under corpses
	atom_flags = RELAY_CLICK
...
/obj/structure/mortar/attack_hand(mob/user as mob)
	if(user.mind && user.skills && user.skills["engineering"] > 40)
		...
		var/fumbling_time = 50 * (user.skills && user.skills["engineering"])
		if(!do_after(user, fumbling_time, 30,src)) return
	if(busy)
...
/obj/structure/mortar/attackby(var/obj/item/O as obj, mob/user as mob)
	...
	if(istype(O, /obj/item/mortal_shell))
		...
		if(user.mind && user.skills && user.skills["engineering"] > 40)
			...
			var/fumbling_time = 50 * (user.skills && user.skills["engineering"])
			if(!do_after(user, fumbling_time, 5,src)) return
		if(busy)
			to_chat(user, "<span class='warning'>Someone else is currently using [src].</span>")
			return
		if(z != 1)
...
/obj/item/mortal_shell
	...
	w_class = 5 //can't stack in backpacks
