...
/obj/structure/cannon/modern/tank/attackby(obj/item/W as obj, mob/M as mob)
	if (broken && istype(W, /obj/item/weapon/weldingtool))
		...
		if (do_after(M, 200, src))
			visible_message("[M] successfully repairs the [src].")
			broken = FALSE
			return
	if (istype(W, ammotype))
		var/obj/item/cannon_ball/shell/tank/TS = W
		if (caliber != TS.caliber && caliber != null && caliber != 0)
			to_chat(M, "<span class = 'warning'>\The [TS] is of the wrong caliber! You need [caliber] mm shells for this cannon.</span>")
			return
		if (loaded)
			to_chat(M, "<span class = 'warning'>There's already a [loaded] loaded.</span>")
			return
		...
		if (found_loader == FALSE && istype(src, /obj/structure/cannon/modern/tank))
			to_chat(M, "<span class = 'warning'>You need to be at the loader's position to load \the [src].</span>")
			return FALSE
...
/obj/structure/cannon/attack_hand(var/mob/attacker)
	if (can_assemble && !assembled)
		to_chat(attacker, "<span class = 'warning'>Assemble the cannon first.</span>")
		return
	else
		interact(attacker)
...
/obj/structure/cannon/interact(var/mob/m)
	...
	if (found_gunner == FALSE && istype(src, /obj/structure/cannon/modern/tank))
		to_chat(user, "<span class = 'warning'>You need to be at the gunner's position to fire.</span>")
		user = null
		return
	if (!anchored)
...
/obj/structure/cannon/Topic(href, href_list, hsrc)
	...
	if (!locate(user) in range(1,src))
		to_chat(user, "<span class = 'danger'>Get behind the cannon to use it.</span>")
		return FALSE
	...
	if (istype(src, /obj/structure/cannon/modern/tank))
		...
		if (found_gunner == FALSE)
			to_chat(user, "<span class = 'warning'>You need to be at the gunner's position to fire \the [src].</span>")
			return FALSE
	...
	if (href_list["fire"])
		...
		if (!loaded)
			to_chat(user, "<span class = 'danger'>There's nothing in \the [src].</span>")
			return
					...
					if (hit)
						...
						spawn (10)
							if (explosion)
								...
								if (target_area == AREA_INSIDE && !target_area.arty_act(25))
